...
async function initComponents() {
    const config = await (0, env_config_provider_1.createDotEnvConfigComponent)({ path: ['.env.defaults', '.env'] }, process.env);
    const cors = {
    ...
    const metrics = await (0, metrics_1.createMetricsComponent)(metrics_2.metricDeclarations, {
        config,
    });
    ...
    const server = await (0, http_server_1.createServerComponent)({ config, logs }, { cors });
    (0, http_tracer_component_1.createHttpTracerComponent)({ server, tracer });
    (0, http_requests_logger_component_1.instrumentHttpServerWithRequestLogger)({ server, logger: logs });
    await (0, metrics_1.instrumentHttpServerWithMetrics)({ metrics, server, config });
    const subgraph = await (0, thegraph_component_1.createSubgraphComponent)({ config, logs, fetch, metrics }, subgraphURL);
    const batchSubgraph = await (0, thegraph_component_1.createSubgraphComponent)({ config, logs: batchLogs, fetch, metrics }, subgraphURL);
    const rentals = await (0, component_5.createRentalsComponent)({ config, fetch, logger: logs });
    ...
    const map = await (0, component_4.createMapComponent)({ config, api, batchApi, tracer });
    ...
    const renderMiniMap = await (0, mini_map_renderer_1.createMiniMapRendererComponent)({ map });
    const renderEstateMiniMap = await (0, mini_map_renderer_1.createEstatesRendererComponent)({ map });
    return {
        config,
        api,
        batchApi,
